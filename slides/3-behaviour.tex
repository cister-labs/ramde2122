...
\\[3mm]
\typerule{res}{P\trans\alpha P'}{\crt P L\trans\alpha \crt{P'}L}~$\faded{\alpha%,\ainv{\alpha}
                                                                  \notin L}$
\msep
...
Two LTS should be \alert{equivalent} if they cannot be distinguished by interacting with them.
...
\begin{block}{Definition}
Let $T = \pair{S, \Act,  \rra}$ be a labelled transition system. The set of \alert{traces} $\Tr{s}$, for $s \in S$ is the minimal set  satisfying
\begin{align*}
\text{\structure{(1)}}\; \;  &  \epsilon \in \Tr{s}\\
\text{\structure{(2)}}\; \;  &  a \cdot \sigma \in \Tr{s}\; \imp\; \rcb{\exists}{s'}{s' \in S}{s \rtran{a} s'\, \e\, \sigma \in \Tr{s'}}
\end{align*}
\end{block}
...
\begin{block}{Definition}
Two states $s, r$ are \alert{trace equivalent} iff  $ \Tr{s} =  \Tr{r}$\\
...
\begin{exampleblock}{\exercise Find bisimulations}
\begin{equation*}
\xymatrix{
