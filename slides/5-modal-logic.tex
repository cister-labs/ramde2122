...
\setLecture{5}{Dynamic Logic \& Verification}
...
\small
\doExercise{Express the following properties in Process Logic}{
\exerciseBack
  ~\\[-5mm]
\begin{itemize}
\item \alert{inevitability of $a$}: \only<2->{\structure{$\pv{-}{\true} \e \nc{-a}{\false}$}}
\item \alert{progress}: %\only<3->{\structure{$\pv{-}{\true}$}}
\item \alert{deadlock or termination}: %\only<4->{\structure{$\nc{-}{\false}$}}
