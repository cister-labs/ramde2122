...
\begin{frame}
  ...
  \begin{block}{Rewritten in EARS}
   \earss{the user drank less than 2 consecutive coffees}{vending machine}{not serve tea}
  \end{block}
\end{frame}
...
\begin{frame}
  ...
  \end{block}
  \begin{block}{Rewritten in EARS}
   \earse{\alert{$a$} was done followed by a \alert{$b$}}{system}{allow the user to do another \alert{$a$}}
  \end{block}
\end{frame}
...
\begin{frame}
  ...
  \end{block}
  \begin{block}{Rewritten in EARS}
   {\bf \color{mygreen} while} it is allocated to a service, \earse{it has collected the passenger}{taxi}{plan the route}
  \end{block}
\end{frame}
...
\begin{frame}
  ...
  \begin{block}{Requirement 7}
    The used \structure{must have} \alert{coffee} after the \alert{coffee button} is pressed.
  \end{block}
  \begin{block}{Type}
