...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item since then, it has been adopted by numerous organizations (Bosch, Honeywell, Intel, Rolls-Royce and Siemens) and included in the requirements engineering curricula of many universities (China, France, Sweden, UK, USA, {\bf and now Portugal {\Large\smiley{}}}).  
  \end{itemize}
  \end{block} 
\end{frame}
%
...
\begin{frame}
  ...
  \begin{block}{Requirement 6}
    The user \structure{can only have} \alert{coffee} after the \alert{coffee button} is pressed.
  \end{block}
  \begin{block}{Type}
   Event Driven
  \end{block}
...
\begin{frame}
  ...
  \begin{block}{Requirement 7}
    The user \structure{must have} \alert{coffee} after the \alert{coffee button} is pressed.
  \end{block}
  \begin{block}{Type}
   Two event driven requirements
  \end{block}
...
%\begin{frame}
%  \frametitle{Expected Tree Structure}
%  \begin{center}
%  {\bf \alert{The target document tree is as below}}\\\vspace{1cm}
%  \begin{tikzpicture}
%\node {REQ}
%    child {node {REQ001}}
%    child {node {TST}
...
%
%\end{slide}
%>>>>>>> 38e4645262e30efa5bcd99f47b2624d081feceae
...
\begin{frame}
  ...
  Lets consider a vending machine with 2 products, apples and bananas, costing 1\euro~ and 2\euro~ respectively. Its users have only 1\euro~ and 2\euro~ coins to interact with the machine. Now, write using EARS patterns the following requirements:
  \begin{itemize}
  \item The user must be able to get apples and bananas;
  \item The machine accepts up to 3\euro, and not more than that;
